...
$n \le 111$.  Certified lower bounds at $n = 2 \times 10^9$ give
$a(n)/n^2 \ge 0.49996$.
\end{abstract}
...
\begin{proof}
  ...
  the string $\mathtt{1}^{\floor{n/2}}\mathtt{0}^{\ceil{n/2}}$
  has
  $\floor{n/2}\ceil{n/2} + \floor{n/2} + 1 > n$ distinct values for
  every $n \ge 2$, as counted in Theorem~\ref{thm:lower}.  Hence
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item The separator theorem is deliberately stated as a proved local
    obstruction.  Stronger separator patterns observed in the data are
    recorded separately as observations and conjectures.
\end{enumerate}
...
$L(s) = \max_t \ell_t$ and $K(s) = \max_t m_t$.

The \emph{first difference} is $\Da(n) = a(n) - a(n{-}1)$ and the
\emph{second difference} is
$\DDa(n) = a(n) - 2a(n{-}1) + a(n{-}2)$.
...
\begin{proof}
  ...
  $f(s) = m(m+1) + 1 = (n^2 - 1)/4 + n/2 + 1/4 > n^2/4 + n/2$.
\end{proof}
...
\begin{remark}\label{rem:optimal-constant}
  The constant $13/4$ is not optimal.  Choosing
  $k = \ceil{\sqrt{3n/2}}$ and tracking the partition variance more
  carefully yields $a(n) \ge n^2/2 - \sqrt{6}\,n^{3/2} + O(n)$,
  with $\sqrt{6} \approx 2.449$.
\end{remark}
...
\begin{proof}
  Take $s_n$ from~\eqref{eq:string}.  By Theorem~\ref{thm:quant},
...
\begin{proof}
  Let $z_n = \zeros(s_n^*)$.  Since $a(n) \ge 2$ for $n \ge 2$ (e.g.,
  $f(\mathtt{10}) = 3$), the optimal string has $z_n \ge 1$ for
  $n \ge 5$ (as $f(\mathtt{1}^n) = n < a(n)$).  By
  Theorem~\ref{thm:quant-sharp} and Lemma~\ref{lem:zero-pos}:
  \[
    \frac{n^2}{2} - \frac{13}{4}\,n^{3/2}
    \le a(n)
...
\begin{remark}\label{rem:convex-upper}
  ...
  $n$ (see Section~\ref{sec:observations}).
\end{remark}
...
\begin{remark}
  ...
  $\DDa(n) = O(n^{3/4})$ and cannot reach the empirical bound
  $\DDa(n) \le 2$ without a fundamentally different structural
  argument.
\end{remark}
...
\begin{lemma}[SAM savings]\label{lem:sam-savings}
  ...
  integer $d$ such that for every binary string $w \in \{0,1\}^d$,
  the word $p[i..k] \cdot w$ has value already in $V_0(p)$ (i.e., the
  SAM state reached from the suffix $p[i..k]$ has a complete binary
  subtree of depth~$d$).  Then
  \[
...
The computation runs in $O(n \log n)$ time, or $O(n)$ using the DivSufSort
library~\cite{pydivsufsort} for suffix array construction.
...
\begin{enumerate}[nosep]
  ...
  \item At a leaf ($k = n$), evaluate $f(s)$ exactly and update $L$
    if $f(s) > L$.
...
\quad \textbf{if} $k = n$: \quad $L \gets \max(L, f(p))$; \quad \textbf{return} \\
...
\begin{remark}[Implementation and reproducibility]
  ...
  and pruned.  The companion sequences A112510, A112511, and A156025
  are then obtained respectively as the minimum, maximum, and
  cardinality of this list; A156022--A156024 follow from the
...
\begin{proof}
  Fuller's A$^*$ search certifies the range $1 \le n \le 80$.  For
  $81 \le n \le 111$, the branch-and-bound computation described in
  Section~\ref{sec:bb-solver}, run in \texttt{collect\_all} mode, uses
  the certified values for smaller residuals as exact upper bounds
  $B(m)=a(m)$.  Hence the upper bound $U(p)$ is certified at every
  ...
  recorded.  Taking the minimum, maximum, and cardinality of this list
  gives A112510, A112511, and A156025, respectively.
\end{proof}
...
Since $f(s)$ is computed exactly at every step, the final value is a
rigorous lower bound on~$a(n)$.
...
\begin{tabular}{rrl}
  ...
  $2 \times 10^9$      & $1{,}999{,}841{,}302{,}297{,}432{,}600$  & $0.49996$ \\
  \bottomrule
\end{tabular}
\end{center}
...
\begin{remark}
  ...
  $n = 14{,}400$.  The computational lower bounds substantially
  exceed the theoretical bound at moderate~$n$, confirming that the
  constant $13/4$ in the $n^{3/2}$ term is far from tight.
\end{remark}
...
\begin{enumerate}[label=(\roman*)]
  ...
  \item \textbf{Alphabet generalisation.}
    ...
    The deficit $n(n{+}1)/2 - a_q(n)$ appears to shrink dramatically
    for $q \ge 3$ (roughly $O(n \log n)$ versus $\Theta(n^{3/2})$
    for $q = 2$), since short-substring collisions vanish once the
    alphabet exceeds~$2$.  Determining the precise deficit order for
    $q \ge 3$ remains open.
\end{enumerate}
