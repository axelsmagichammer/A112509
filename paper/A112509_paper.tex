...
\begin{proof}
  ...
  has $z_n \ge 1$ in the range relevant to the asymptotic claim.  By
...
\begin{lemma}[SAM savings]\label{lem:sam-savings}
  ...
  for every $0 \le j \le d$ and every binary string
...
\begin{proof}
  ...
  the source table.  For $81 \le n \le 111$, the branch-and-bound
  computation described in Section~\ref{sec:bb-solver}, run in
  \texttt{collect\_all} mode, uses
...
\begin{remark}
  ...
  exceed the theoretical bound at moderate~$n$, indicating that the
  constant $13/4$ in the $n^{3/2}$ term is far from tight in practice.
\end{remark}
