...
$r=M-(kq+k(k-1)/2)$, so $0\le r<k$.  Define
\[
  b_i=\begin{cases}q+k-i+1,&1\le i\le r,\\q+k-i,&r<i\le k.\end{cases}
\]
Then $b_1>b_2>\cdots>b_k$, $\sum b_i=M$, and $M\ge k(k+1)/2$ gives
$q\ge1$, so $b_k\ge1$.  Every $b_i$ lies in the interval $(q,q+k]$,
which contains $M/k$ and has width $k$.
\end{proof}
...
\end{equation}
Since $n\ge64$ we have $k\ge2$ and $M\ge n/2\ge k(k+1)/2$ for all
$n\ge64$.  Choose $b_1>b_2>\cdots>b_k\ge1$ with $\sum b_i=M$ and
